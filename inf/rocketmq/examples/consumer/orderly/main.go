...
func main() {
	...
	// #1 创建PushConsumer
	{
		...
		if c, err = rocketmq.NewPushConsumer(
			...
			consumer.WithConsumerModel(consumer.Clustering),                //可以省略
			consumer.WithConsumeFromWhere(consumer.ConsumeFromFirstOffset), //默认是从LastOffset
		); err != nil {
	...
	// #2 进行订阅，并指定回调函数
	{
		...
		var consume = func(ctx context.Context, msgs ...*primitive.MessageExt) (consumer.ConsumeResult, error) {
			...
			//业务.1) 循环处理收集参数msgs
			orderlyCtx, exists := primitive.GetOrderlyCtx(ctx)
			fmt.Println("exists:", exists, "orderlyCtx:", orderlyCtx)
